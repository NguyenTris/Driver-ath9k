...
static u32 ath_txq_get_dynamic_drop_threshold(struct ath_softc *sc,
					      struct ath_txq *txq,
					      unsigned int q,
					      unsigned int pkt_len)
{
	u32 threshold = txq->axq_drop_threshold;

	if (q < IEEE80211_NUM_ACS && sc->tx.ac_qdepth_drop_threshold[q])
		threshold = sc->tx.ac_qdepth_drop_threshold[q];

	/*
	 * Keep latency-sensitive traffic under tighter queue occupancy.
	 * We apply a stricter threshold for VO/VI and small packets.
	 */
	if (q < IEEE80211_NUM_ACS &&
	    (q == IEEE80211_AC_VO || q == IEEE80211_AC_VI || pkt_len <= 256))
		threshold = max_t(u32, threshold / 2, ATH_AGGR_MIN_QDEPTH);
...
struct ath_txq *ath_txq_setup(struct ath_softc *sc, int qtype, int subtype)
{
	...
	if (!ATH_TXQ_SETUP(sc, axq_qnum)) {
		...
		txq->axq_dropped = 0;
		txq->axq_drop_threshold = ATH_MAX_QDEPTH_DROP_THRESHOLD;
		txq->axq_tx_inprogress = false;
		sc->tx.txqsetup |= 1<<axq_qnum;

		if (qtype == ATH9K_TX_QUEUE_DATA &&
		    subtype >= 0 && subtype < IEEE80211_NUM_ACS) {
			switch (subtype) {
			case IEEE80211_AC_VO:
				sc->tx.ac_qdepth_drop_threshold[subtype] =
					ATH_MAX_QDEPTH_DROP_THRESHOLD_VO;
				break;
			case IEEE80211_AC_VI:
				sc->tx.ac_qdepth_drop_threshold[subtype] =
					ATH_MAX_QDEPTH_DROP_THRESHOLD_VI;
				break;
			case IEEE80211_AC_BK:
				sc->tx.ac_qdepth_drop_threshold[subtype] =
					ATH_MAX_QDEPTH_DROP_THRESHOLD_BK;
				break;
			case IEEE80211_AC_BE:
			default:
				sc->tx.ac_qdepth_drop_threshold[subtype] =
					ATH_MAX_QDEPTH_DROP_THRESHOLD_BE;
				break;
			}
			txq->axq_drop_threshold =
				sc->tx.ac_qdepth_drop_threshold[subtype];
		}
			       ...
			       struct ath_atx_tid *tid, struct sk_buff *skb)
{
	...
	struct list_head bf_head;
	struct ath_buf *bf = fi->bf;
	u32 drop_threshold = txq->axq_drop_threshold;

	if (drop_threshold && txq->axq_depth >= drop_threshold) {
		/*
		 * Late drop path: ath_tx_setup_buffer() already dma_map_single()'d
		 * skb data. Unmap before freeing to avoid DMA mapping leaks.
		 */
